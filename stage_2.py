...
if __name__ == '__main__':
    ...
    for i in range(1):
        bg_image_name=('test.jpg')
        background = Image.open(bg_image_name)
        l=len("the descriptions does here")
        if l >= 40:
            description=("the descriptions does here".split())
            description.insert(5,"\n")
            description=" ".join(description)
        else:
            description="the descriptions does here"
        background = write_image(background,colors[bgcolors[randint(0,9)]],"$ "+"hacker loi",description,foreground=foreground)
